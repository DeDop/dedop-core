...
class AzimuthProcessingAlgorithm(BaseAlgorithm):
    ...
    def __call__(self, isps, wavelength_ku, method=AzimuthProcessingMethods.dynamic, weighting=AzimuthWeighting.disabled):
        """
        ...
        if method == AzimuthProcessingMethods.dynamic:
            ...
            self.compute_approximate_method(isps, wavelength_ku)
        ...
        # azimuth processing with exact method
            ...
            self.compute_exact_method(isps, wavelength_ku)
    ...
    def compute_phase_shift(self, isp, nadir_beam_angle, wavelength_ku, waveform_phase_shift):
        """
        :param isp:
        ...
        for pulse_index in range(self.chd.n_ku_pulses_burst):
            beam_angle_phase = np.exp(-2j *  2. * self.cst.pi * wavelength_ku *
                                      norm(isp.vel_sat_sar) * cos(nadir_beam_angle) *
                                      isp.pri_sar_pre_dat * pulse_index)
            waveform_phase_shift[pulse_index, :] =\
                beam_angle_phase * isp.waveform_cor_sar[pulse_index, :]

    def compute_fft_azimuth_dimension(self, waveform_phase_shift, waveform_fft_azimuth):
        """
        :param waveform_phase_shift:
        ...
        for sample_index in range(self.chd.n_samples_sar):
            ...
            for pulse_index in range(self.chd.n_ku_pulses_burst):
                ...
                waveform_fft_azimuth[sample_index, pulse_index] =\
